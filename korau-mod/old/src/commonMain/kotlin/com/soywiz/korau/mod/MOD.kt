...
class MOD {
	...
	class Interpreter(val mod: MOD) {
				...
				if (period > 0) {
					...
					samplesLock {
						...
						for (n in 0 until ((rate / rowsPerSecond)).toInt()) {
							...
							samples.add((sample * volumeFloat).toInt().toShort())
			...
			samplesLock {
				for (n in 0 until (channels.map { it.samples.size }.max() ?: 0)) {
					...
					if (count == 0) {
						...
						samples.add(((accum / count) * 0.3).toInt().toShort())
					}
				}
				for (channel in channels) {
		...
		suspend fun play() {
			AudioStream.generator(rate, 1) {
				samplesLock {
					val out = samples.toShortArray()
					samples.clear()
					write(AudioSamplesInterleaved(1, out.size, out))
					out.size
				}
				true
			}.playAndWait()
