...
//  if (!this.amigaperiods) this.channel[ch].slidetospeed*=4;
		...
		return object : AudioStream(samplerate, 2) {
			...
			override suspend fun read(out: AudioSamples, offset: Int, length: Int): Int {
				...
				for (n in 0 until len) {
					out[0, m] = lbuf[n].toShortSample(mixval)
					out[1, m] = rbuf[n].toShortSample(mixval)
					m++
				}
	...
	fun Double.toShortSample(mixval: Double): Short {
		...
		return (r * Short.MAX_VALUE).clamp(Short.MIN_VALUE.toDouble(), Short.MAX_VALUE.toDouble()).toInt().toShort()
	}
